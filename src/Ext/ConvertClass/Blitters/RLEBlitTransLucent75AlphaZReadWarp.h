...
DEFINE_RLE_BLITTER(RLEBlitTransLucent75AlphaZReadWarp, BlitterPixelWordOnly)
{
public:
	inline explicit RLEBlitTransLucent75AlphaZReadWarp(WORD* data, WORD mask, int shadecount) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
					...
					if (srcv)
					{
						...
						while (remaining >= ChunkSize)
						{
							...
							if (zMask)
							{
								...
								const __m512i warpQuarter = _mm512_and_si512(_mm512_srli_epi32(warpColors, 2), blendMask);
								const __m512i srcQuarter = _mm512_and_si512(_mm512_srli_epi32(srcColors, 2), blendMask);
								const __m512i result32 = _mm512_add_epi32(_mm512_mullo_epi32(warpQuarter, three), srcQuarter);
						...
						while (remaining--)
						{
							...
							if (zval < *zbuf)
								*pDest = 3 * (mask & (pDest[warp] >> 2)) + (mask & (pPaletteData[*pRunSrc | pAData[*abuf]] >> 2));
					...
					if (srcv)
					{
						...
						while (remaining >= ChunkSize)
						{
							...
							if (_mm256_movemask_epi8(zMask32))
							{
								...
								const __m256i warpQuarter32 = _mm256_and_si256(_mm256_srli_epi32(warpColors, 2), blendMask32);
								const __m256i srcQuarter32 = _mm256_and_si256(_mm256_srli_epi32(srcColors, 2), blendMask32);
								const __m256i result32 = _mm256_add_epi32(_mm256_add_epi32(_mm256_add_epi32(warpQuarter32, warpQuarter32), warpQuarter32), srcQuarter32);
						...
						if (srcv)
						{
							...
							while (remaining >= ChunkSize)
							{
								...
								if (_mm_movemask_epi8(zMask16))
								{
									...
									const __m128i warpQuarter16 = _mm_and_si128(_mm_srli_epi16(warp16, 2), blendMask16);
									const __m128i srcQuarter16 = _mm_and_si128(_mm_srli_epi16(srcColor16, 2), blendMask16);
									const __m128i result16 = _mm_add_epi16(_mm_add_epi16(_mm_add_epi16(warpQuarter16, warpQuarter16), warpQuarter16), srcQuarter16);
		...
		auto handler = [pPaletteData, pAData, mask, warp](WORD& dest, byte srcv, int zbase, WORD zbufv, byte zadjustv, WORD abufv)
		{
			...
			if (zval < zbufv)
				dest = 3 * (mask & ((&dest)[warp] >> 2)) + (mask & (pPaletteData[srcv | pAData[abufv]] >> 2));
		};
