...
DEFINE_RLE_BLITTER(RLEBlitTransRemapDestZReadWrite, BlitterPixelByteOnly)
{
public:
	inline explicit RLEBlitTransRemapDestZReadWrite(BYTE* data) noexcept
	{
		this->RemapDest = data;
	}
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
					...
					while (remaining >= ChunkSize)
					{
						...
						if (zMask)
						{
							...
							for (int lane = 0; lane < ChunkSize; ++lane)
							{
								if ((zMask & (3 << (lane * 2))) != 0)
								{
									pDest[lane] = pRemapDest[pDest[lane]];
		...
		auto handler = [pRemapDest](BYTE& pDest, byte srcv, int zbase, WORD& zbufv, byte zadjustv)
		{
