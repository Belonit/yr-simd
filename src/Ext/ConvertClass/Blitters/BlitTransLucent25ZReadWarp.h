...
DEFINE_BLITTER(BlitTransLucent25ZReadWarp, BlitterPixelWordOnly)
{
public:
	inline explicit BlitTransLucent25ZReadWarp(WORD* data, WORD mask) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
		...
		while (len--)
		{
			...
			{
				if (byte idx = *src)
					*pDest = (mask & (pDest[warp] >> 2)) + 3 * (mask & (pPaletteData[idx] >> 2));
			}
			++src;
			++pDest;
