...
DEFINE_BLITTER(BlitTransLucent25AlphaZReadWrite, BlitterPixelWordOnly)
{
public:
	inline explicit BlitTransLucent25AlphaZReadWrite(WORD* data, WORD mask, int shadecount) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
		...
		while (len--)
		{
			...
			if (zval < zbufv && idx)
			{
				*pDest = (mask & (*pDest >> 2)) + 3 * (mask & (pPaletteData[idx | pAData[*abuf]] >> 2));
				zbufv = zWriteValue;
			}
