			...
			while (len >= ChunkSize)
			{
				...
				if (zMask)
				{
					...
					if (activeMask)
					{
						...
						const __m512i bgHalf = _mm512_and_si512(_mm512_srli_epi32(bgColors, 1), blendMask);
						const __m512i srcHalf = _mm512_and_si512(_mm512_srli_epi32(srcColors, 1), blendMask);
						const __m512i result32 = _mm512_add_epi32(bgHalf, srcHalf);
			...
			while (len >= ChunkSize)
			{
				...
				if (_mm_movemask_epi8(zMask16))
				{
					...
					if (_mm_movemask_epi8(activeMask16))
					{
						...
						const __m128i bgHalf16 = _mm_and_si128(_mm_srli_epi16(bg16, 1), blendMask16);
						const __m128i srcHalf16 = _mm_and_si128(_mm_srli_epi16(srcColor16, 1), blendMask16);
						const __m128i result16 = _mm_add_epi16(bgHalf16, srcHalf16);
