...
DEFINE_RLE_BLITTER(RLEBlitTransLucent25ZReadWarp, BlitterPixelWordOnly)
{
public:
	inline explicit RLEBlitTransLucent25ZReadWarp(WORD* data, WORD mask) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
				...
				while (len > 0)
				{
					...
					if (srcv)
					{
						...
						while (remaining--)
						{
							...
							if (zval < *zbuf)
							{
								*pDest = (mask & (pDest[warp] >> 2)) + 3 * (mask & (pPaletteData[*pRunSrc] >> 2));
							}
		...
		auto handler = [pPaletteData, mask, warp](WORD& dest, byte srcv, int zbase, WORD zbufv, byte zadjustv)
		{
			...
			if (zval < zbufv)
				dest = (mask & ((&dest)[warp] >> 2)) + 3 * (mask & (pPaletteData[srcv] >> 2));
		};
