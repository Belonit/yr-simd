...
class RLEBlitTransLucent50AlphaZReadFix final : public RLEBlitTransLucent50AlphaZRead<T, Level>
{
public:
	inline explicit RLEBlitTransLucent50AlphaZReadFix(WORD* data, WORD mask, int shadecount) noexcept
		: RLEBlitTransLucent50AlphaZRead<T, Level>(data, mask, shadecount)
	{
	}

	virtual ~RLEBlitTransLucent50AlphaZReadFix() override final = default;
	...
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
				...
				if (srcv)
				{
					...
					while (remaining >= ChunkSize)
					{
						...
						if (zMask)
						{
							...
							const __m512i result32 = _mm512_add_epi32(halfDiff32, _mm512_and_si512(srcColors, destColors));
					...
					while (remaining--)
					{
						...
						if (zval < *zbuf)
						{
							...
							*pDest = static_cast<T>(blendedColor);
						}
					...
					if (srcv)
					{
						...
						while (remaining >= ChunkSize)
						{
							...
							if (_mm_movemask_epi8(zMask16))
							{
								...
								const __m128i result16 = _mm_add_epi16(halfDiff16, _mm_and_si128(srcColor16, dest16));
		...
		auto handler = [pPaletteData, pAData](WORD& dest, byte srcv, int zbase, WORD zbufv, byte zadjustv, WORD abufv)
		{
			...
			if (zval < zbufv)
			{
				...
				const unsigned int blendedColor = (((srcColor ^ destColor) & RGB565BlendMask) >> 1) + (srcColor & destColor);
				dest = static_cast<T>(blendedColor);
			}
		};
...
static_assert(sizeof(RLEBlitTransLucent50AlphaZReadFix<WORD, Simd::Level::Scalar>) == sizeof(RLEBlitTransLucent50AlphaZRead<WORD, Simd::Level::Scalar>));
