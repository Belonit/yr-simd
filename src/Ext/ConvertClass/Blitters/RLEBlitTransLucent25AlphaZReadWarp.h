...
DEFINE_RLE_BLITTER(RLEBlitTransLucent25AlphaZReadWarp, BlitterPixelWordOnly)
{
public:
	inline explicit RLEBlitTransLucent25AlphaZReadWarp(WORD* data, WORD mask, int shadecount) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
					...
					if (srcv)
					{
						...
						while (remaining >= ChunkSize)
						{
							...
							if (zMask)
							{
								...
								const __m512i result32 = _mm512_add_epi32(warpQuarter, _mm512_mullo_epi32(srcQuarter, three));
						...
						while (remaining--)
						{
							...
							if (zval < *zbuf)
								*pDest = (mask & (pDest[warp] >> 2)) + 3 * (mask & (pPaletteData[*pRunSrc | pAData[*abuf]] >> 2));
					...
					if (srcv)
					{
						...
						while (remaining >= ChunkSize)
						{
							...
							if (_mm256_movemask_epi8(zMask32))
							{
								...
								const __m256i result32 = _mm256_add_epi32(warpQuarter32, _mm256_add_epi32(_mm256_add_epi32(srcQuarter32, srcQuarter32), srcQuarter32));
						...
						if (srcv)
						{
							...
							while (remaining >= ChunkSize)
							{
								...
								if (_mm_movemask_epi8(zMask16))
								{
									...
									const __m128i result16 = _mm_add_epi16(warpQuarter16, _mm_add_epi16(_mm_add_epi16(srcQuarter16, srcQuarter16), srcQuarter16));
		...
		auto handler = [pPaletteData, pAData, mask, warp](WORD& dest, byte srcv, int zbase, WORD zbufv, byte zadjustv, WORD abufv)
		{
			...
			if (zval < zbufv)
				dest = (mask & ((&dest)[warp] >> 2)) + 3 * (mask & (pPaletteData[srcv | pAData[abufv]] >> 2));
		};
