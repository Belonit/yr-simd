...
DEFINE_BLITTER(BlitTransLucent75ZReadWarp, BlitterPixelWordOnly)
{
public:
	inline explicit BlitTransLucent75ZReadWarp(WORD* data, WORD mask) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
		...
		while (len--)
		{
			...
			{
				if (byte idx = *src)
					*pDest = 3 * (mask & (pDest[warp] >> 2)) + (mask & (pPaletteData[idx] >> 2));
			}
			++src;
			++pDest;
