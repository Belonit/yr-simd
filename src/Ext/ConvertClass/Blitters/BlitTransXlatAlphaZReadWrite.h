...
private:
	__forceinline void Blit_Impl(void* dst, byte * src, int len, int zval, WORD * zbuf, WORD * abuf, int alvl, int warp)
	{
		...
		if constexpr (Level == Simd::Level::AVX512 && CompileAvx512)
		{
			...
			const __m256i zWriteVec = _mm256_set1_epi16(static_cast<short>(zval));
			...
			while (len >= ChunkSize)
			{
				...
				if (zMask)
				{
					...
					if (activeMask)
					{
						...
						const __m256i result16 = _mm512_cvtusepi32_epi16(resultColors);
		...
		if constexpr (Level == Simd::Level::AVX2 && CompileAvx2)
		{
			...
			const __m128i zWriteVec16 = _mm_set1_epi16(static_cast<short>(zval));
			...
			while (len >= ChunkSize)
			{
				...
				if (_mm_movemask_epi8(zMask16))
				{
					...
					if (_mm_movemask_epi8(activeMask16))
					{
						...
						const __m128i result16 = _mm_load_si128(reinterpret_cast<const __m128i*>(resultArray));
						const __m128i oldValue16 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(pDest));
						const __m128i blended16 = Sse2_BlendU16(oldValue16, result16, activeMask16);
						const __m128i oldZ16 = _mm_loadu_si128(reinterpret_cast<const __m128i*>(zbuf));
						const __m128i zWrite16 = Sse2_BlendU16(oldZ16, zWriteVec16, activeMask16);
						_mm_storeu_si128(reinterpret_cast<__m128i*>(pDest), blended16);
						_mm_storeu_si128(reinterpret_cast<__m128i*>(zbuf), zWrite16);
					}
				}
		...
		while (len--)
		{
			...
			if (zval < zbufv && idx)
			{
				*pDest = pPaletteData[idx | pAData[*abuf]];
