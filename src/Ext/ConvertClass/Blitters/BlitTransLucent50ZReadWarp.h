...
DEFINE_BLITTER(BlitTransLucent50ZReadWarp, BlitterPixelWordOnly)
{
public:
	inline explicit BlitTransLucent50ZReadWarp(WORD* data, WORD mask) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
		...
		while (len--)
		{
			...
			{
				if (byte idx = *src)
					*pDest = (mask & (pDest[warp] >> 1)) + (mask & (pPaletteData[idx] >> 1));
			}
			++src;
			++pDest;
