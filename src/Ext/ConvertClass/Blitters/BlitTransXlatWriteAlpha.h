...
DEFINE_BLITTER(BlitTransXlatWriteAlpha, BlitterPixelWordOnly)
{
public:
	...
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
		...
		if constexpr (Level == Simd::Level::AVX512 && CompileAvx512)
		{
			...
			while (len >= ChunkSize)
			{
				...
				if (activeMask)
				{
					...
					const __m512i alphaValues = _mm512_min_epi32(_mm512_add_epi32(srcValues, zvalVec), max255);
		...
		if constexpr (Level == Simd::Level::AVX2 && CompileAvx2)
		{
			...
			while (len >= ChunkSize)
			{
				...
				{
					const __m256i alphaValues = _mm256_min_epi32(_mm256_add_epi32(srcValues32, zvalVec32), max255);
		...
		{
			if (byte idx = *src++)
				*abuf = static_cast<WORD>(std::min(idx + zval, 255));
			++abuf;
			++pDest;
