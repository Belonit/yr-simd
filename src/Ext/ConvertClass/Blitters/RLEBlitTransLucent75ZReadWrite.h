...
DEFINE_RLE_BLITTER(RLEBlitTransLucent75ZReadWrite, BlitterPixelWordOnly)
{
public:
	inline explicit RLEBlitTransLucent75ZReadWrite(WORD* data, WORD mask) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int line, int zbase, WORD* zbuf, WORD* abuf, int alvl, int warp, byte* zadjust)
	{
		...
		// Scalar
		auto handler = [pPaletteData, mask](WORD& dest, byte srcv, int zbase, WORD& zbufv, byte zadjustv)
		{
