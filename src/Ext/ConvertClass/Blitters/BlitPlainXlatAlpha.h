...
DEFINE_BLITTER(BlitPlainXlatAlpha, BlitterPixelWordOnly)
{
public:
	inline explicit BlitPlainXlatAlpha(WORD* data, int shadecount) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
		...
		while (len--)
		{
			*pDest++ = pPaletteData[*src++ | pAData[*abuf++]];
