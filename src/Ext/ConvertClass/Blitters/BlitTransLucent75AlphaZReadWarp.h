...
DEFINE_BLITTER(BlitTransLucent75AlphaZReadWarp, BlitterPixelWordOnly)
{
public:
	inline explicit BlitTransLucent75AlphaZReadWarp(WORD* data, WORD mask, int shadecount) noexcept
	{
...
private:
	__forceinline void Blit_Impl(void* dst, byte* src, int len, int zval, WORD* zbuf, WORD* abuf, int alvl, int warp)
	{
			...
			while (len >= ChunkSize)
			{
				...
				if (zMask)
				{
					...
					if (activeMask)
					{
						...
						const __m512i result32 = _mm512_add_epi32(_mm512_add_epi32(_mm512_add_epi32(bgQuarter, bgQuarter), bgQuarter), srcQuarter);
			...
			while (len >= ChunkSize)
			{
				...
				if (_mm256_movemask_epi8(zMask32))
				{
					...
					if (_mm256_movemask_epi8(activeMask32))
					{
						...
						const __m256i result32 = _mm256_add_epi32(_mm256_add_epi32(_mm256_add_epi32(bgQuarter32, bgQuarter32), bgQuarter32), srcQuarter32);
			...
			while (len >= ChunkSize)
			{
				...
				if (_mm_movemask_epi8(zMask16))
				{
					...
					if (_mm_movemask_epi8(activeMask16))
					{
						...
						const __m128i result16 = _mm_add_epi16(_mm_add_epi16(_mm_add_epi16(bgQuarter16, bgQuarter16), bgQuarter16), srcQuarter16);
		...
		while (len--)
		{
			...
			byte idx = *src++;
			if (zval < zbufv && idx)
				*pDest = 3 * (mask & (pDest[warp] >> 2)) + (mask & (pPaletteData[idx | pAData[*abuf]] >> 2));
